...
| `NATURAL_ANSWER_V2_ENABLED` | 阶段 11 自然回答开关；需与语义验证开关同时启用。旧变量 `GROUNDED_SYNTHESIS_ENABLED` 仅作为兼容别名。 |
| `SEMANTIC_VERIFIER_ENABLED` | 阶段 11 独立语义验证开关；不可用时回退确定性回答。旧变量 `SEMANTIC_VERIFICATION_ENABLED` 仅作为兼容别名。 |
| `NATURAL_ANSWER_V2_ROLLOUT_PERCENT` | 可选，0–100 的稳定灰度比例，默认 100。旧变量 `GROUNDED_SYNTHESIS_ROLLOUT_PERCENT` 只用于仍采用旧别名开关的部署。 |
